		...
		.then((docSnapshot) => {
			if (docSnapshot.data()) {
				if (docSnapshot.data().game5 !== undefined) { 
					document.getElementById("label-choice-seasongame95").innerHTML = `<label class="choice">${docSnapshot.data().game5} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
		  ...
		  .then((docSnapshot) => {
			if (docSnapshot.exists) {
				week.update({
					...
					game5: document.getElementById('seasongame95').value
				}).then(function() {
					success();
				}).catch(err => {
