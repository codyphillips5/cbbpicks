		...
		.then((docSnapshot) => {
			if (docSnapshot.data()) {
				if (docSnapshot.data().game6 !== undefined) { 
					document.getElementById("label-choice-seasongame126").innerHTML = `<label class="choice">${docSnapshot.data().game6} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
					document.getElementById("label-choice-seasongame127").innerHTML = `<label class="choice">${docSnapshot.data().game7} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
					document.getElementById("label-choice-seasongame128").innerHTML = `<label class="choice">${docSnapshot.data().game8} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
					document.getElementById("label-choice-seasongame129").innerHTML = `<label class="choice">${docSnapshot.data().game9} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
		  ...
		  .then((docSnapshot) => {
			if (docSnapshot.exists) {
				week.update({
					...
					game6: document.getElementById('seasongame126').value,
					game7: document.getElementById('seasongame127').value,
					game8: document.getElementById('seasongame128').value,
					game9: document.getElementById('seasongame129').value
				}).then(function() {
					success();
				}).catch(err => {
