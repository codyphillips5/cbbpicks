		...
		.then((docSnapshot) => {
			if (docSnapshot.data()) {
				if (docSnapshot.data().game1 !== undefined) { 
					document.getElementById("label-choice-seasongame111").innerHTML = `<label class="choice">${docSnapshot.data().game1} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
					document.getElementById("label-choice-seasongame112").innerHTML = `<label class="choice">${docSnapshot.data().game2} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
		  ...
		  .then((docSnapshot) => {
			if (docSnapshot.exists) {
				week.update({
					...
					game1: document.getElementById('seasongame111').value,
					game2: document.getElementById('seasongame112').value
				}).then(function() {
					success();
				}).catch(err => {
