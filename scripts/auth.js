		...
		.then((docSnapshot) => {
			if (docSnapshot.data()) {
				if (docSnapshot.data().game1 !== undefined) { 
					document.getElementById("label-choice-seasongame121").innerHTML = `<label class="choice">${docSnapshot.data().game1} <span class="glyphicon glyphicon-plusglyphicon glyphicon-check"></span></label>`
		  ...
		  .then((docSnapshot) => {
			if (docSnapshot.exists) {
				week.update({
					...
					game1: document.getElementById('seasongame121').value
				}).then(function() {
					success();
				}).catch(err => {
